function [output] = lsqr_op_sine(input,opt,Ncoeffs,X,Y,GG,k,sensors,q,u_tot)
...
if strcmp(opt,'notransp')    
    %input (coeffs(:)) in R^{Ncoeffs^3}
    %output (2Nd*Ntgt,1) in R
    
    %from sine series to domain values - >this is correct
    inp_aux = reshape(input,Ncoeffs,Ncoeffs);
    aux_domain = q_domain(X,Y,11,Ncoeffs,inp_aux);
    inp_domain = aux_domain;    
...
%     size(outputaux)
    ...
    %projection into sine modes
    cf = filter_adj(N,outputaux,Ncoeffs);
